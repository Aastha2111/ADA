...
public class PrimsMethod {
	 ...
	 {     
	     ...
	     for (int count=0; count<V-1;count++) {       
	    	//u=min_index
	         ...
	         for (int v = 0; v < V; v++) 
	             if (graph[u][v]!=0 && mstSet[v]==false && graph[u][v]<key[v]) //smallerst key vert not in mst set
	             { 
	                 mstlist[v] = u; 
	                 key[v] = graph[u][v]; 
	             } 
	...
	FileWriter writer = new FileWriter("C:\\Users\\K L Madaan\\Desktop\\java\\ADA-SORTING\\src\\PrimsText", true);
	BufferedWriter bufferedWriter = new BufferedWriter(writer);
	 System.out.println("AASTHA-19CSU003");
	 System.out.println("");
	
		PrimsMethod t = new PrimsMethod(); 
